---
title: "Defining the Future of haematopoiesis: Mmrn1 as a Novel Marker for Long-Term haematopoietic Stem Cells"
engine: knitr
execute:
  echo: false
  include: true
  ...
  warning: false
format: html 
# the code below is set to html output due to plotly interactive graphs, if one wants a pdf output then png images of the plotly objects in place # of them in the panel figures. 
# I found more control of the plotly objects looks can be achieved with a html output and performing Command P and exporting as PDF. 
editor: visual
...
```{css, echo=FALSE}
  body {
    ...
    font-size: 18px; /* Sets the overall font size to 13px */
    text-align: justify; /* Justifies all text within the body */
    line-height: 1.4; /* Sets the line height to 1.6 */
  }
...
# Fit the ANOVA model
anova_model <- aov(ENSMUSG00000054641 ~ cell_type, data = filtered_er_cell_types_long_prioritised)

# Perform ANOVA
anova_result_filtered <- anova(anova_model)

# Perform Tukey's HSD test
tukey_result_filtered <- TukeyHSD(anova_model)

# Print the Tukey's HSD test results
print(tukey_result_filtered)
```
...
Mmrn1_subpop_plot <- ggplot(average_expression_with_sem_filtered, aes(x = cell_type, y = Average_Expression, fill = cell_type)) +
  ...
  scale_fill_manual(values = c("yellow", "darkgreen", "blue", "green", "orange", "red")) +  
  ...
  annotate("text", x = 2.5, y = 8, label = "****", size = 7) +
  annotate("text", x = 5, y = 7.5, label = "****", size = 7) +
  annotate("text", x = 3.5, y = 7, label = "****", size = 7) +
  annotate("text", x = 3, y = 6.5, label = "****", size = 7) +
  annotate("text", x = 4.5, y = 6, label = "****", size = 7) +
...
# Fit the ANOVA model
anova_model <- aov(ENSMUSG00000054641 ~ cell_type, data = filtered_er_cell_types_long_prioritised_different)
...
Mmrn1_subpop_MPP_plot <- ggplot(average_expression_with_sem_filtered_new, aes(x = cell_type, y = Average_Expression, fill = cell_type)) +
  ...
  scale_y_continuous(limits = c(NA, 9.5)) + 
  annotate("segment", x = 3, xend = 4, y = 8.3, yend = 8.3, colour = "black", size = .5) + 
  annotate("segment", x = 1, xend = 3, y = 6.8, yend = 6.8, colour = "black", size = .5) +
  annotate("segment", x = 1, xend = 2, y = 7.3, yend = 7.3, colour = "black", size = .5) +  
  annotate("segment", x = 3, xend = 5, y = 7.8, yend = 7.8, colour = "black", size = .5) +  
  annotate("segment", x = 2, xend = 3, y = 6.3, yend = 6.3, colour = "black", size = .5) +   
  annotate("segment", x = 4, xend = 2, y = 8.8, yend = 8.8, colour = "black", size = .5) +
  annotate("segment", x = 5, xend = 2, y = 9.3, yend = 9.3, colour = "black", size = .5) +
  annotate("text", x = 3.5, y = 8.55, label = "NS", size = 4) +
  annotate("text", x = 1.5, y = 7.3, label = "***", size = 7) +
  annotate("text", x = 4, y = 7.8, label = "****", size = 7) +
  annotate("text", x = 2, y = 7.05, label = "NS", size = 4) +
  annotate("text", x = 2.5, y = 6.55, label = "NS", size = 4) +
  ...
  annotate("text", x = 3, y = 9.05, label = "NS", size = 4) +
  annotate("text", x = 3.5, y = 9.3, label = "***", size = 7) 
...
ESLAM_mmrn1_plot <- ggplot(gene_expression_SEM, aes(x = new_cell, y = Average_Expression, fill = Gene)) +
  ...
  annotate("segment", x = 0.75, xend = 1, y = 1.5, yend = 1.5, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 0.75, xend = 0.75, y = 1.5, yend = 1.4, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 1, xend = 1, y = 1.5, yend = 1.4, colour = "black", linewidth = 0.5) +
  annotate("text", x = 0.875, y = 1.74, label = "NS", size = 2.5) +
  annotate("segment", x = 1, xend = 1.25, y = 2, yend = 2, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 1, xend = 1, y = 2, yend = 1.9, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 1.25, xend = 1.25, y = 2, yend = 1.9, colour = "black", linewidth = 0.5) +
  annotate("text", x = 1.125, y = 2.24, label = "NS", size = 2.5) +
  annotate("segment", x = 0.75, xend = 1.25, y = 2.5, yend = 2.5, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 0.75, xend = 0.75, y = 2.5, yend = 2.4, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 1.25, xend = 1.25, y = 2.5, yend = 2.4, colour = "black", linewidth = 0.5) +
  annotate("text", x = 1, y = 2.74, label = "NS", size = 2.5) +
  annotate("segment", x = 1.75, xend = 2, y = 8, yend = 8, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 1.75, xend = 1.75, y = 8, yend = 7.9, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 2, xend = 2, y = 8, yend = 7.9, colour = "black", linewidth = 0.5) +
  annotate("text", x = 1.875, y = 8.24, label = "NS", size = 2.5) +
  annotate("segment", x = 2, xend = 2.25, y = 8.5, yend = 8.5, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 2, xend = 2, y = 8.5, yend = 8.4, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 2.25, xend = 2.25, y = 8.5, yend = 8.4, colour = "black", linewidth = 0.5) +
  annotate("text", x = 2.125, y = 8.64, label = "****", size = 5) +
  annotate("segment", x = 1.75, xend = 2.25, y = 9, yend =  9, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 1.75, xend = 1.75, y = 9, yend = 8.9, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 2.25, xend = 2.25, y = 9, yend = 8.9, colour = "black", linewidth = 0.5) +
  annotate("text", x = 2, y = 9.14, label = "****", size = 5) +
  annotate("segment", x = 2.75, xend = 3, y = 1.3, yend = 1.3, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 2.75, xend = 2.75, y = 1.3, yend = 1.2, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 3, xend = 3, y = 1.3, yend = 1.2, colour = "black", linewidth = 0.5) +
  annotate("text", x = 2.875, y = 1.54, label = "NS", size = 2.5) +
  annotate("segment", x = 3, xend = 3.25, y = 1.9, yend = 1.9, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 3, xend = 3, y = 1.9, yend = 1.8, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 3.25, xend = 3.25, y = 1.9, yend = 1.8, colour = "black", linewidth = 0.5) +
  annotate("text", x = 3.125, y = 2.14, label = "NS", size = 2.5) +
  annotate("segment", x = 2.75, xend = 3.25, y = 2.4, yend = 2.4, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 2.75, xend = 2.75, y = 2.4, yend = 2.3, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 3.25, xend = 3.25, y = 2.4, yend = 2.3, colour = "black", linewidth = 0.5) +
  annotate("text", x = 3, y = 2.64, label = "NS", size = 2.5) +
  annotate("segment", x = 3.75, xend = 4, y = 2.8, yend = 2.8, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 3.75, xend = 3.75, y = 2.8, yend = 2.7, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 4, xend = 4, y = 2.8, yend = 2.7, colour = "black", linewidth = 0.5) +
  annotate("text", x = 3.875, y = 2.94, label = "*", size = 5) +
  annotate("segment", x = 4, xend = 4.25, y = 3.3, yend = 3.3, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 4, xend = 4, y = 3.3, yend = 3.2, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 4.25, xend = 4.25, y = 3.3, yend = 3.2, colour = "black", linewidth = 0.5) +
  annotate("text", x = 4.125, y = 3.44, label = "****", size = 5) +
  annotate("segment", x = 3.75, xend = 4.25, y = 3.8, yend = 3.8, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 3.75, xend = 3.75, y = 3.8, yend = 3.7, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 4.25, xend = 4.25, y = 3.8, yend = 3.7, colour = "black", linewidth = 0.5) +
  annotate("text", x = 4, y = 3.94, label = "****", size = 5) +
  annotate("segment", x = 4.75, xend = 5, y = 6.2, yend = 6.2, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 4.75, xend = 4.75, y = 6.2, yend = 6.1, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 5, xend = 5, y = 6.2, yend = 6.1, colour = "black", linewidth = 0.5) +
  annotate("text", x = 4.875, y = 6.44, label = "NS", size = 2.5) +
  annotate("segment", x = 5, xend = 5.25, y = 6.7, yend = 6.7, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 5, xend = 5, y = 6.7, yend = 6.6, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 5.25, xend = 5.25, y = 6.7, yend = 6.6, colour = "black", linewidth = 0.5) +
  annotate("text", x = 5.125, y = 6.84, label = "****", size = 5) +
  annotate("segment", x = 4.75, xend = 5.25, y = 7.2, yend = 7.2, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 4.75, xend = 4.75, y = 7.2, yend = 7.1, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 5.25, xend = 5.25, y = 7.2, yend = 7.1, colour = "black", linewidth = 0.5) +
  annotate("text", x = 5, y = 7.34, label = "****", size = 5) +
  annotate("segment", x = 6, xend = 6.25, y = 2.2, yend = 2.2, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 6, xend = 6, y = 2.2, yend = 2.1, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 6.25, xend = 6.25, y = 2.2, yend = 2.1, colour = "black", linewidth = 0.5) +
  annotate("text", x = 5.875, y = 2.94, label = "*", size = 5) +
  annotate("segment", x = 5.75, xend = 6, y = 2.7, yend = 2.7, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 6, xend = 6, y = 2.7, yend = 2.6, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 5.75, xend = 5.75, y = 2.7, yend = 2.6, colour = "black", linewidth = 0.5) +
  annotate("text", x = 6.125, y = 2.34, label = "****", size = 5) +
  annotate("segment", x = 5.75, xend = 6.25, y = 3.2, yend = 3.2, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 5.75, xend = 5.75, y = 3.2, yend = 3.1, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 6.25, xend = 6.25, y = 3.2, yend = 3.1, colour = "black", linewidth = 0.5) +
  annotate("text", x = 6, y = 3.34, label = "***", size = 5) +
  annotate("segment", x = 6.75, xend = 7, y = 3.9, yend = 3.9, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 6.75, xend = 6.75, y = 3.9, yend = 3.8, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 7, xend = 7, y = 3.9, yend = 3.8, colour = "black", linewidth = 0.5) +
  annotate("text", x = 6.875, y = 4.14, label = "NS", size = 2.5) +
  annotate("segment", x = 7, xend = 7.25, y = 4.4, yend = 4.4, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 7, xend = 7, y = 4.4, yend = 4.3, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 7.25, xend = 7.25, y = 4.4, yend = 4.3, colour = "black", linewidth = 0.5) +
  annotate("text", x = 7.125, y = 4.54, label = "****", size = 5) +
  annotate("segment", x = 6.75, xend = 7.25, y = 4.9, yend = 4.9, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 6.75, xend = 6.75, y = 4.9, yend = 4.8, colour = "black", linewidth = 0.5) +
  annotate("segment", x = 7.25, xend = 7.25, y = 4.9, yend = 4.8, colour = "black", linewidth = 0.5) +
  annotate("text", x = 7, y = 5.04, label = "****", size = 5) 
...
qPCR_plot <- ggplot(qPCR_data_processed, aes(x = cell_type_grouped, y = Mmrn1_expression, fill = cell_type_grouped)) +
  ...
  theme(
    ...
    plot.subtitle = element_text(size = 20, hjust = 0.5, vjust = 1), # Bigger plot subtitle, 
    legend.text = element_text(size = 17), legend.title = element_text(size = 20), # Increase legend text size
    axis.line = element_line(color = "black"),
    legend.key.size = unit(1, "cm"), # Center and increase title size
    plot.margin = margin(t = 0, r = 0, b = 30, l = 0, unit = "pt")
  ) + scale_fill_manual(values = custom_colors) +  ylim(0, 13.5) +
  annotate("text", x = 1, y = 6.82, label = "4.861", size = 5) +
  annotate("text", x = 2, y = 9.72, label = "5.266", size = 5) +
  annotate("text", x = 3, y = 2.32, label = "1.411", size = 5) +
  annotate("segment", x = 1, xend = 2, y = 10.4, yend = 10.4, colour = "black", size = 0.9) +
  annotate("segment", x = 2, xend = 3, y = 11.5, yend = 11.5, colour = "black", size = 0.9) +
  annotate("segment", x = 1, xend = 3, y = 12.6, yend = 12.6, colour = "black", size = 0.9) +
  annotate("text", x = 1.5, y = 10.85, label = "P = 0.929", size = 5) +
  annotate("text", x = 2.5, y = 11.95, label = "P = 0.4048", size = 5) +
  annotate("text", x = 2, y = 13.05, label = "P = 0.08899", size = 5) 
...
**Project Director**: Dr Jillian Barlow

**Project Title**: Investigating the Differentiation of Stem Cells in Healthy Bone Marrow using RNA Sequencing

<br> <br> <br> <br> <br>

# Abstract

haematopoietic stem cells (HSCs) sit at the apex of the differentiation hierarchy, marked by their capacity for both self-renewal and differentiation into all blood lineages. Research is ongoing to enhance HSC engraftment as a therapeutic strategy to aid in the treatment of haematopoietic diseases. The identification of genes expressed specifically in HSCs may help improve methods of stem cell isolation. In this study, we investigate *Multimerin-1* (*Mmrn1)* as a potential novel marker of long-term haematopoietic stem cells (LT-HSCs) through analysis of single cell RNA sequencing data collected by Nesterowa et al., 2016, complemented by quantitative PCR (qPCR). Analysis of over 1600 single HSPC transcriptomes revealed the differential expression between specific cell populations, indicating a change in the transcriptional landscape during haematopoiesis. This also revealed *Mmrn1* to be distinctly upregulated in LT-HSCs compared to other progenitor populations. We attempted to validate these findings by qPCR, confirming high expression in the LT-HSC and MPP populations of cells. These findings indicate Mmrn1 may have significant implications for improving HSC purification protocols and enhancing transplantation outcomes. Additionally, previous findings have reported *Mmrn1* upregulation in many cancers and an association with Leukaemia stem cells. We propose Mmrn1, known for its adhesive functions and extensive role in the extracellular matrix (ECM), may have a role in the bone marrow matrix for sustaining Long-term repopulating cells. The insights from this study not only propose a novel marker for stem cell biology but also contribute to the understanding of cellular dynamics within the bone marrow micro-environment.

Abstract Word Count: 248
Report Word Count: 3,476

<br> <br> <br> <br> <br> <br> <br> <br> <br> <br> <br>

# Introduction

haematopoietic stem cells (HSCs) are at the apex of the differentiation hierarchy and are defined by their ability to differentiate and produce the entire haematopoietic system upon transplantation. In fact, for over 60 years HSC transplantation has been the main treatment for haematological malignancies, whereby success is strongly correlated to the number of HSCs transplanted [@eaves2015]. Long-term haematopoietic stem cells (LT-HSCs) sit within this HSC pool and maintain blood production and respond to inflammation, blood loss, and injury [@gur-cohen2016]. LT-HSCs are found in the bone marrow (BM) microenvironment, the main site of adult haematopoiesis, in a non-proliferative, quiescent mode via adhesive interactions [@morrison2014; @boulais2015; @mendelson2014]. They are of much interest due to their capacity to support long-term haematopoietic reconstitution meaning they are capable of repopulating the bone marrow in functional transplantation assays, which requires high self-renewal ability, developmental potential and homing [@gur-cohen2016; @notta2011]. The identification and characterization of these functional LT-HSCs has been a long-standing goal in the field. Thus far, strategies for the purification of HSCs utilise fluorescence activated cell sorter (FACS)--based protocols that rely on the detection of cell-surface antigens that are differentially expressed on HSCs [@berman1985; @spangrude1989]. Whilst numerous seminal studies have uncovered the origins of HSCs and the key factors that influence their functionality [@bertrand2010; @hamey2017; @baron2018; @hou2020], the selection of HSCs for research primarily depends on various cell surface markers, which functions remain poorly understood. Indeed, studies using single-cell and serial transplantation techniques have shown significant heterogeneity in both the production of mature cells and self-renewal durability of individual HSCs isolated using these markers [@beerman2010; @dykstra2007; @goodell1996; @morita2010]. Its believed this functional heterogeneity is being controlled via intrinsic and extrinsic mechanisms [@copley2013; @wilkinson2013], but the specific molecular mechanisms that dictate HSC expansion and maintenance are largely unknown. Over the course of the last 30 years, protocols have been developed for the prospective isolation of increasingly refined haematopoietic stem and progenitor cell (HSPC) populations toward this LT-HSC phenotype. Currently, there are four widely used HSC isolation protocols, whereby ESLAM cells (CD150^+^ EPCR^+^CD48^−^CD45^+^) have been found to have one of the highest long term repopulation potentials [@wilson2015; @adolfsson2001; @weksberg2008; @kent2009; @kent2016]. ESLAM cells utilise endothelial protein C receptor (EPCR), also known as CD201 and encoded by the gene Procr, as one of the few known markers that overtly identifies haematopoietic stem cells within murine bone marrow [@balazs2006; @iwasaki2010; @zhou2016; @fares2017; @subramaniam2019; @papa2020; @anjos-afonso2022; @rix2022; @lin2023]. When used alongside CD34^+^ HSPCs, which are considered to sustain human haematopoiesis, EPCR significantly enhances the isolation of cells which have high repopulating and self-renewal abilities in humans [@anjos-afonso2022; @balazs2006; @fares2017; @iwasaki2010; @papa2020; @rix2022; @subramaniam2019; @zhou2016]. Moreover, CD150 is used as another marker utilised by ESLAM cells, to identify HSCs with high self-renewal and reconstituting potential [@thomas1989; @shivtiel2008; @kiel2005; @papathanasiou2009; @beerman2010; @morita2010]. The combination of EPCR and CD150 therefore provides an effective strategy for isolating functional HSCs allowing for purities of more than 50%. However, no current method yields a single homogeneous HSC population. Additionally, the restrictive gating used in purification protocols often omits transitional cells that may be crucial in our understanding of haematopoiesis [@wilson2015; @kent2009; @zhang2005; @nestorowa2016]. Recently, the development of single-cell profiling techniques have allowed the ability to profile transitional cells and resolve the heterogeneity within these populations at large numbers [@bendall2014; @jaitin2014; @mahata2014]. This should enable the identification of novel HSC markers, allowing for a deeper insight into human HSC biology and facilitate further purification of HSCs for clinical applications discussed earlier [@nestorowa2016]. Here we identify *Multimerin-1* as a potential novel marker of LT-HSCs. Mmrn1 (human is abbreviated: *MMRN1*; mouse is abbreviated: *Mmrn1*) is a member of the EMILIN/Multimerin family of proteins, found in platelets, endothelial cells (EC), megakaryocytes, and the extracellular matrix (ECM) [@hayward1991; @hayward1991; @hayward1993; @hayward1995; @adam2005; @colombatti2011]. Mmrn1 is highly conserved in mammals and, importantly, murine and human orthologs are functionally very similar in spite of sequence differences (only 67% sequence similarity) [@hayward1995; @jeimy2008; @reheman2010]. It is a polymeric, vascular adhesive protein which forms large, disulphide-linked polymers of trimers that can reach millions of Daltons in size [@hayward1991; @hayward1991; @hayward1995; @jeimy2008]. Most of the literature surrounding Mmrn1 focus on its adhesive functions in the ECM, mediated via its Arg-Gly-Asp motif (RGD) located at its N-terminus [@adam2005]. In addition to its adhesive functions, Mmrn1 interacts with various integrin receptors through its conserved leucine zipper motif, facilitating its binding to platelet-activating receptors, platelet integrins and chemokine receptors [@jeimy2008]. These observations relate to the differential expression of *MMRN1*, observed in numerous cancers, as chemokines are known to play significant roles in tumour progression [@gay2011; @wojtukiewicz2017]. Moreover, upregulation of the *Mmrn1* gene is associated with increased risk of relapse and poor survival in acute myeloid leukaemia (AML) patients [@laszlo2015]. More recently it has also been used to distinguish leukaemia stem cells from leukaemia progenitor cells [@laszlo2015]. To date there have been many studies documenting differential expression of the *MMRN1* gene in numerous cancers but its actual molecular mechanism is unknown [@saini2023]. We determined *Mmrn1* differential expression in LT-HSCs by analysing single cell RNA-sequencing data collected by Nestorowa et al [@nestorowa2016]. By analysing over 1600 single HSPC transcriptomes (profiled into LT-HSCs, HSPCs and Progenitor cells using FACS index sorting) we have been able to explore the transcriptional landscape of early HSC differentiation. Here, we find *Mmrn1* shows a significant association with the LT-HSC population and differential expression compared with the HSPC and Progenitor populations making it a prime candidate for a novel marker of LT-HSCs. Further analysis via quantitative PCR validates this trend with high expression seen in the LT-HSC and MPP populations when compared with the progenitor population. Collectively, our study provides a comprehensive comparison of *Mmrn1* expression in the different bone marrow populations indicating it may be a promising candidate for a LT-HSC marker and worth further exploration.

# Methods

#### **Quantitative PCR.**

QPCR was performed using the SYBR Green Master Mix ([ThermoFisher, Waltham, MA, USA)](https://www.fishersci.com/shop/products/applied-biosystems-sybr-fast-green-master-mix-6/p-4926995) on the StepOne™ Real-Time PCR System ([Applied Biosystems, Foster City, CA,USA](https://www.thermofisher.com/order/catalog/product/4376599?SID=srch-srp-4376599)). The qPCRs were held at an initial holding stage at 95°C for 10 minutes to activate the DNA polymerase, followed by 40 cycles of denaturation (15 s at 95°C) and annealing (60 s at 60°C). cDNA was obtained by Dr Jillian Barlow from the bone marrow of groups of five healthy 12-16 week old C57 BL/6 mice. FACS was used to isolate LT-HSCs (Lin^-^Sca1^+^C^-^kit^+^Flk2^-^CD34^-^), MPPs (Lin^-^Sca1^+^C^-^kit^+^Flk2^+^CD34^+^), and PROGs (Lin^-^Sca1^-^C^-^kit^+^) using the markers listed. RNA extraction was carried out using the PicoPure RNA extraction kit ([Applied Biosystems, Fisher](https://www.thermofisher.com/order/catalog/product/KIT0204)). RNA was then converted into cDNA using Superscript IV reverse transcriptase (Invitrogen) according to manufacturer's instructions. cDNA expression was measured as relative abundance and normalised to the housekeeping gene *Hprt* using the DeltadeltaCT method. All the experiments were conducted in triplicate. *Mmrn1* Primer sequences were designed using the NCBI primer-blast design tool[@ye2012] using the manufacturer Merck. The sequences: forward primer 5'-AACTTCATCTGCGCTTGCCG-3' and reverse primer 5'-CAGGATAGGACCAGGTGCCG-3'. The housekeeping gene *Hprt* primer sequences were designed by Merck. The sequences: forward primer 5'- GTTGGATACAGGCCAGACTTTGTTG-3' and reverse primer 5'- GAGGGTAGGCTGGCCTATAGGCT-3'.

#### **Single Cell RNA Sequencing Computational Analysis.**

All computational analysis was performed in the R programming environment ([https://www.r-project.org](https://www.r-project.org/)). We took RNA-seq data are from an Illumina machine. The raw data was extracted as Reads in FASTQ files and the counts of these were used to quantify the expression of genes, containing the sequence and a quality score for each base. The reads were filtered and trimmed on the basis of the quality score then aligned to a reference genome/transcriptome. The counts were then normalised to account for differences in the sequencing depth and gene/transcript/genome length before statistical analysis[@rand2023]. The dataset was then presented as log~2~ normalised values and filtered for cell surface genes [@bausch-fluck2015].

Values are presented as means ± SEM. Statistical significance was determined by one-way ANOVA for overall expression differences of *Mmrn1* across the multiple cell types. With subsequent pairwise comparisons facilitated through Tukey's Honest Significant Difference (HSD) test to identify specific differences between groups. For direct comparison between two genes, unpaired students t-tests were utilised. P-values \< 0.05 were considered significant and shown in each figure. 
For more information of specific packages and data analysis techniques used please visit: [Supplementary_Data_Analysis_Methods.html](https://joewomersley.github.io/Supplementary_Methods/Supplementary_Data_Analysis_Methods.html). To reproduce any of the data collected please visit: <https://github.com/JoeWomersley/REPORT>
...
Previous single cell (sc)RNASeq has mapped the transcriptional landscape of mouse haematopoietic stem and progenitor cells (HSPCs), revealing nearly continuous differentiation, marked by distinct gene expression changes [@klein2015; @nestorowa2016; @trapnell2014]. This analysis included over 1600 single HSPC transcriptomes (profiled into 155 LT-HSCs, 701 HSPCs and 798 Progenitor cells using FACS index sorting). We have been able to explore the transcriptional landscape of early HSC differentiation using a sub section of this data, filtered for 280 surfaceome genes [@bausch-fluck2015; @nestorowa2016].

We then conducted screening of this data and ensured the gene expression across the cells was consistent across the different cell types **([Figure_S1 - 3](#panelS1)**). Using 3D principal component analysis (PCA), we were able to reduce the high-dimensional gene expression data into few principal components [@ringnér2008]. The analysis showed distinct gene expression profiles for the LT-HSC, HSPC and Progenitor populations ([**Figure_1A**](#panel1)) and indicated differential expression of genes corresponding with the different phenotypes ([**Figure_S4**](#panelS4)). To analyse this further we conducted a differential expression (DE) analysis ([**Figure_1B**](#fig-panel1)). This revealed 64 DE expressed genes between Prog vs HSPC, 41 between lthsc vs HSPC and, 78 between prog vs lthsc. Overall, we identified 101 genes which were differentially expressed; absolute log~2~-transformed fold change (FC) \> 1 with False Discover Rate (FDR) \< 0.05 ([**Table_1**](#top-differentially-expressed-genes)). The most DE genes being *Procr, Mmrn1, Esam, Cd48 and Cd34. Mmrn1*, which has not been extensively studied or reported to be differentially expressed in haematopoietic stem cells (HSCs), showed one of the largest summary log~2~ fold changes, with expression particularly high in the LT-HSC population, suggesting a potentially novel finding. Unsupervised clustering of the 1656 cells and the top 50 differentially expressed genes showed subdivision of the cells into four main clusters ([**Figure_1C**](#fig-panel1)). Most interestingly, cluster 4 contained the majority of the LT-HSCs, which clustered with the HSPCs and is represented by genes such as *Procr*, Mmrn1 and *Esam*. Cluster 1 contained mainly Progenitors as well as HSPCs and is characterised by expression of genes such as *Clec12a*, *sell* and *Itgal*. Cluster 2 and 3 are both composed of mainly Progenitors and are differentiated by higher expression of several genes, including *Epdr1*, *Emilin2* and *Tfrc* in cluster 3. With the combination of this hierarchical clustering, differential gene analysis and principal component analysis we were able to show marked differential expression between cell types, indicating a change in the transcriptional landscape during haematopoiesis.

#### **Gene expression can map haematopoietic cell types.**

Previously we have shown distinct gene expression profiles for the LT-HSC, HSPC and Progenitor populations ([**Figure_1A**](#panel1), [**Figure_S4**](#panelS4)). However, the progenitor and HSPC populations can further be defined into 6 sub-populations using index sorting data based on flow cytometry markers ([**Figure_2A**](#panel2)**)**[@nestorowa2016]. We visualised the top differentially expressed genes, highlighting cells in red when log~2~ absolute expression of a select gene is higher than 3 ([**Figure_2B**](#fig-panel2)). We created two web resources: [https://plotly.com/\~Jwomersley /20.embed](https://plotly.com/~Jwomersley/20.embed) to visualise and compare 12 of the top differentially expressed genes using the 3D PCA plot ([**Figure_2A**](#panel2)) and [https://plotly.com/\~Jwomersley/29. embed](https://plotly.com/~Jwomersley/29.embed) to visualise all sub-population cell types. Our data shows that high *Mmrn1* gene expression overlaps with the LT-HSC population.
...
```{css, echo=FALSE}
/* allows for box around the figure panels */
.black-box {
    border: 2px solid black;  /* Adjust thickness as needed */
    padding: 5px;  /* Adjust padding inside the border */
    margin: 5px;  /* Optional: adds space around the box */
}
```

```{css}
/* adjust margins to ensure the plots lines up correcty in the panel */
...
#panel1 .plotly {
  margin-bottom: -200px;  /* Adjust the negative margin to bring plots closer, this is a work around due to issues with Plotly working with ggplot */
}
#fig-panel1 {
  display: flex;
  flex-direction: column;
...
#fig-panel1 .pheatmap {
  margin-top: -500px; /* Adjust the negative margin to bring plots closer, this is a work around due to issues with Plotly working with ggplot */
...
```{r}
# adjusting the margins, width and height of the plot. above we have ensured the ncol is set to 3 to ensure the plots are displayed in a 3 column layout 
# style = "display: flex; justify-content: space-around; align-items: center; flex-wrap: wrap;" is used to ensure the plots are displayed in a row and 
# wrap to the next row if the screen size is too small.
...
```{r}
# adjusting the margins, width and height of the plot. 
...
# adjusting the height of the plot (fig.height). above we have ensured the ncol is set to 3 to ensure the plots are displayed in a 3 column layout 
# style = "display: flex; justify-content: space-around; align-items: center; flex-wrap: wrap;" is used to ensure the plots are displayed in a row and 
# wrap to the next row if the screen size is too small.
...
#A bove we specified that they are not figures this is due to Plotly and ggplot not working correctly together. now we are specifying that this is a 
# figure by specifying fig-panel1, this allows for the generation of automatic numbering of the figure legend in the document. so even though the code 
# specificity three separate panels, they are all grouped under the same figure legend and by adjusting the margins they grop together in the render.

heatmap_subpopulation_clustering

```

**3D Principal Component Analysis, hierarchical clustering and volcano plots can be used to visualise changes in gene expression across haemopoietic stem and progenitor cell differentiation**. (**A)** 3D PCA plots of cell surface protein expression profiles across LTHSC (Green), HSPC (Red) and Progenitor (Blue) cell types. The PCA was performed on normalised log~2~ expression data of surfaceome genes, whereby each dot represents a single cell and projected based on its surfaceome gene expression profile through the first 3 principal components (PC1, PC2 and PC3). Visualised: <https://plotly.com/~Jwomersley/27.embed>. **(B)** Volcano plots depicting the differential expressed genes (DEGs) between HSPCs and LT-HSCs (left panel), between Progenitors and LT-HSCs (middle panel) and between progenitors and HSPCs (right panel). The x-axis summarizes the log fold change (logFC) in gene expression, while the y-axis represents the negative log~10~ of the FDR. Genes with significant differential expression are highlighted above the horizontal dashed line, indicating FDR \< 0.05. Genes of the highest summary.logFC are highlighted and labelled. **(C)**  Unsupervised hierarchical clustering of gene expression data for all cells. showing gene expression of the top 50 differentially expressed genes in all cells with the sub-population definitions applied. The cells split into 4 major clusters (cluster 1, purple; cluster 2, lightblue; cluster 3, orange; cluster 4, deeppink). Each cell in the heatmap represents the expression level of a gene in a specific cell type. Gene expression is displayed on a log~2~ scale from blue to red (low to high). All 1654 cells are visualised along the X axis and the top 50 differentially expressed genes are shown on the Y axis, as calculated using a statistical model which finds the summary.logFC. Both the genes and cells were clustered using the ward method.
:::
:::
...
top_25_genes <- top_differentially_expressed_genes_no_duplicates %>%
  slice_head(n = 25)

# Generating a static table with kable from the knitr package
knitr::kable(top_25_genes,
...
Table 1: **Top 25 Differentially expressed genes**. The differentially expressed genes are defined by a differential gene analysis. FDR corresponds to false discovery rate using the Benjamini-Hochberg procedure. Due to the multiple comparisons problem in gene expression analysis, the p-value is adjusted to control the false discovery rate, which is the expected proportion of false positives. Summary.logFC corresponds to the log fold change which is the measure of the difference in expression levels of a gene between two of the three cell-types. Higher Summary.logFC values indicate a higher level of differential expression. Note negative and positive high numbers both correspond with a high level of differential expression - this just corresponds to the swing in expression whereby negative numbers correspond to higher expression in the second cell type in the comparison. The data was also filtered to only show comparisons with a P.value \< 0.05. This comparison negates gene duplicates whereby they are highly differentially expressed between more than one comparison. We have made an interactive table to compare both the top differentially expressed genes with [duplicates](https://joewomersley.github.io/REPORT/results/HTML/dt_top_differentially_expressed_genes_duplicates.html) and [without duplicates](https://joewomersley.github.io/REPORT/results/HTML/dt_top_differentially_expressed_genes_no_duplicates.html).
:::
:::
...
```{css}
/* This sets the margins and to address many issues with the margins below the plots by specifying a large negative bottom margin */
...
#panel2 .plotly, #fig-panel2 .plotly {
    ...
    max-width: calc(33.333% - 15px); /* Calculate width, subtracting margins. this is to solve issues with the plots not sitting together */
  }
```
...
# specify the height and width of the plot again we are treating this not as a figure as we are grouping different plotly plots together
...
```{r, align = "center", out.width='100%'}
# above we have now specifcied it a figure to enable the caption to be added. By not grouping theres together has ebabled the "B" to be added so we can 
# seperate the different plots. 
...
**3D principle component analysis can be used to visualise gene expression of the sub population cell types and match there expression to top DE surfaceome genes**. **(A)** 3D PCA analysis showing difference in gene expression between sub population cell types. Sub population cell types were defined using flow cytometry markers using broad gating and were identified retrospectively by using index sorting data [@nestorowa2016]. Each dot represents a single cell and projected based on its surfaceome gene expression profile through the first three principle components (PC1, PC2 and PC3). MPP populations were coloured: MPP1-Deeppink, MPP2-pink, MPP3-purple. **(B)** 3D PCA analysis showing cells highlighted with high expression of the top 5 differentially expressed genes. Cells are highlighted in red when absolute expression of that gene is greater than 3.0. Interactive version of sub-populations: <https://plotly.com/~Jwomersley/29.embed>. Interactive version looking at the 12 top DE genes: <https://plotly.com/~Jwomersley/20.embed>.
:::
:::
...
To analyse *Mmrn1* expression further, we averaged expression its in the LT-HSC, HSPC and progenitor populations. This analysis demonstrated that LT-HSCs had significantly higher average *Mmrn1* expression compared to the HSPC and Progenitor populations ([**Figure_3A)**](#panel3), aligning with the gene expression profiles previously shown in our PCA ([**Figure_2A-B**](#fig-panel2)). As previously stated, the progenitor and HSPC populations can be further divided into 6 sub-populations. Correspondingly, averaging *Mmrn1* expression across the sub-populations confirmed its high expression remained within the LT-HSC and MPP populations ([**Figure_3B**](#panel3)). The MPP population itself can be subdivided into MPP1, MPP2, MPP3, and a generic MPP category for cells outside the more specific gating parameters. This subdivision revealed substantial variability in *Mmrn1* expression across the subtypes ([**Figure_3C**](#panel3)). MPP1 and the generic MPP populations maintained high expression levels, with MPP3 showing a marked decrease. These findings coincide with our principal component analysis where MPP1 closely paralleled the LT-HSC profile, followed by the generic MPP, MPP2, and MPP3 in descending order of similarity ([**Figure_2B**](#fig-panel2)). Moreover, previous hierarchical clustering placed *Mmrn1* in cluster 4 alongside the MPP and LT-HSC populations ([**Figure_1C**](#fig-panel1)), underscoring its potential as a novel marker of long-term repopulating cells.

#### ***Mmrn1*** **Aligns with *Procr* in Marking Long-Term haematopoietic Stem Cells.**

Hierarchical clustering revealed that *Mmrn1* is closely aligned with *Procr*, a known marker of long-term repopulating cells ([**Figure_1C**](#fig-panel1)). Further comparison using 3D PCA demonstrated that *Mmrn1* and *Procr* are similarly expressed across the entire population of cells, with high expression of *Mmrn1* and *Procr* represented by yellow and low by dark blue. Quantitative analysis showed *Procr* and *Mmrn1* exhibit comparable expression levels across the various sub-populations, including ESLAM cells ([**Figure_3E**](#fig-panel3)). Expression being non-significantly different in all cells, except LMPPs (P = 0.0392) and MEPs (P = 0.0309). The expression of *Slamf1,* a marker used alongside *Procr* for isolating ESLAM cells, did not match that of *Procr* or *Mmrn1*. While its differences with *Procr* and *Mmrn1* were non-significant in CMPs and GMPs, *Slamf1* expression was significantly lower in MPPs and LT-HSCs. The most prominent finding was that *Mmrn1* expression was comparable to *Procr* levels in the ESLAM cells. This high level of expression in cells commonly used for HSC purification underscores *Mmrn1's* overlap with *Procr* expression, reinforcing its potential use as a novel marker for LT-HSCs.
...
The *ex vivo* expansion of HSCs has been a major goal in stem cell biology, with implications for the ability to produce limitless populations of mature blood cells. There has been a major push towards purifying cells with increasing stem cell potential, such as LT-HSCs. Major breakthroughs have been achieved with near to 900-fold *ex vivo* expansion [@wilkinson2019]. But there is marked heterogeneity in the self-renewal capacity of these HSCs and the identity of those that have long term repopulating potential following *ex vivo* HSC expansion remains elusive [@chen2019; @psatha2017]. Here we utilise molecular profiling techniques that have provided a single-cell resolution map of stem cell differentiation with the goal to identify a novel marker of LT-HSCs [@nestorowa2016].
...
# new panel generated as had layout-ncol = 3 and now layout-ncol = 2 is nessesary. 
# Solving many issues with code below by removing the title and text of the axis and allowing for it to have a legend subtitle ("D"). 
...
plot_3D_PCA_Procr <- plot_3D_PCA_Procr_data %>%
  ...
  layout(
    ...
    showlegend = FALSE,  # No legend displayed
    margin = list(l = 0, r = 0, b = 10, t = 30),  # Custom margins (top margin increased for title)
    width = 310,  # Set width of the plot
    height = 300  # Set height of the plot
  )
...
[dt_top_differentially_expressed_genes_no_duplicates.html](https://joewomersley.github.io/REPORT/results/HTML/dt_top_differentially_expressed_genes_no_duplicates.html "dt_top_differentially_expressed_genes_no_duplicates.html")

[dt_top_differentially_expressed_genes_duplicates.html](https://joewomersley.github.io/REPORT/results/HTML/dt_top_differentially_expressed_genes_duplicates.html "dt_top_differentially_expressed_genes_no_duplicates.html")
